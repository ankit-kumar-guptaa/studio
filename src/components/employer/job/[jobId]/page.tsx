...
export default function JobApplicantsPage() {
  ...
  const handleStatusChange = async (application: JobApplication, newStatus: JobApplication['status']) => {
    if (!firestore || !user || !jobId) return;
  
    const appDocRef = doc(firestore, `employers/${user.uid}/jobPosts/${jobId}/applications`, application.id);
    const seekerAppDocRef = doc(firestore, `jobSeekers/${application.jobSeekerId}/applications`, application.jobPostId);
  
    try {
      // Update status in both employer's and seeker's collections
      await Promise.all([
        updateDoc(appDocRef, { status: newStatus }),
        updateDoc(seekerAppDocRef, { status: newStatus })
      ]);
      ...
      console.error("Error updating status in both collections:", error);
    }
  };
